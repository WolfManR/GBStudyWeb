...
namespace MetricsManager.DataBase.Repositories
{
    public class RamMetricsRepository : IRamMetricsRepository
    {
        ...
        public RamMetricsRepository(SQLiteContainer container)
        {
        ...
        public IList<RamMetric> GetByTimePeriod(DateTimeOffset from, DateTimeOffset to, int agentId)
        {
            ...
            using var connection = _container.CreateConnection();
            using var cmd = new SQLiteCommand(connection);
            cmd.CommandText = "SELECT * FROM rammetrics WHERE (agentId=@agentId) and (time > @from) and (time < @to)";
            cmd.Parameters.AddWithValue("@from", fromSeconds);
            cmd.Parameters.AddWithValue("@to", toSeconds);
        ...
        public void Create(RamMetric entity)
        {
            using var connection = _container.CreateConnection();
            using var cmd = new SQLiteCommand(connection);
            cmd.CommandText = "INSERT INTO rammetrics(agentId,time,value) VALUES (@agentId,@time,@value);";
